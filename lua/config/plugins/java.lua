return {
	...
	config = function()
		...
		local nix_config_dir = jdtls_base .. "/config_" .. config_os
		local writable_config_dir = home .. "/.cache/jdtls/config_" .. config_os

		-- Copy config from Nix store to writable location if it doesn't exist
		if vim.fn.isdirectory(writable_config_dir) == 0 then
			vim.fn.mkdir(writable_config_dir, "p")
			vim.fn.system(string.format("cp -r %s/* %s/", nix_config_dir, writable_config_dir))
		end
